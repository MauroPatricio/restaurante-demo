...
import { restaurantAPI, usersAPI, roleAPI, analyticsAPI, currencyAPI } from '../services/api';
import {
...
export default function AdminHub() {
    ...
    const fetchSupportedCurrencies = async () => {
        try {
            const res = await currencyAPI.getSupported();
    ...
    const handleSave = async (section) => {
        ...
        try {
            ...
            if ((section === 'general' || section === 'currencies') && logoFile && section === 'general') {
                ...
                payload.append('settings.isKitchenOpen', formData.general.isKitchenOpen);
                ...
                if (section === 'general' || section === 'currencies') {
                    payload = {
                        ...
                        'settings.isKitchenOpen': formData.general.isKitchenOpen
                    };
    ...
    const tabs = [
        { id: 'general', label: t('tab_general'), icon: Building, color: '#3b82f6', gradient: 'from-blue-500/10 to-transparent' },
        { id: 'users', label: t('tab_users'), icon: Users, color: '#8b5cf6', gradient: 'from-purple-500/10 to-transparent' },
        { id: 'customers', label: t('tab_customers'), icon: UserPlus, color: '#ec4899', gradient: 'from-pink-500/10 to-transparent' },
        { id: 'flows', label: t('tab_flows'), icon: Activity, color: '#10b981', gradient: 'from-emerald-500/10 to-transparent' },
        { id: 'visual', label: t('tab_visual'), icon: Palette, color: '#f59e0b', gradient: 'from-amber-500/10 to-transparent' },
        { id: 'currencies', label: t('tab_currencies'), icon: Coins, color: '#6366f1', gradient: 'from-indigo-500/10 to-transparent' },
        { id: 'security', label: t('tab_security'), icon: Shield, color: '#ef4444', gradient: 'from-red-500/10 to-transparent' },
        { id: 'integrations', label: t('tab_integrations'), icon: Share2, color: '#6366f1', gradient: 'from-indigo-500/10 to-transparent' },
    ];
                ...
                <div style={{ marginBottom: '48px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                            ...
                            <span style={{ fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.2em', color: '#4f46e5' }}>{t('admin_control_center') || 'Admin Control Center'}</span>
                        </div>
                        <h1 style={{ fontSize: '48px', fontWeight: '900', color: '#0f172a', margin: 0, letterSpacing: '-0.05em', lineHeight: 1.1 }}>
                            {t('admin_hub_title').split(' ')[0]} <span style={{ color: '#6366f1' }}>{t('admin_hub_title').split(' ').slice(1).join(' ')}</span>
                        </h1>
                ...
                <div style={{ display: 'flex', gap: '40px' }}>
                    ...
                    <div style={{ width: '320px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div style={{
                            ...
                            <h4 style={{ fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.2em', color: '#94a3b8', marginBottom: '8px' }}>{t('unit_status')}</h4>
                            <p style={{ fontSize: '18px', fontWeight: '700', lineHeight: 1.2, margin: 0 }}>{t('system_operational')}</p>
                            <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <div style={{ width: '8px', height: '8px', background: '#34d399', borderRadius: '50%' }} className="animate-pulse" />
                                <span style={{ fontSize: '10px', fontWeight: '900', color: '#34d399', textTransform: 'uppercase', letterSpacing: '0.2em' }}>{t('synchronized')}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div>
                                ...
                                <div style={{
                                    ...
                                    {t('professional_config', { tab: tabs.find(t => t.id === activeTab)?.label })}
                                </div>
                            </div>
                        ...
                        <div className="p-12 flex-1 overflow-y-auto max-h-[calc(100vh-280px)] custom-scrollbar">
                                    ...
                                    <div style={{ display: 'flex', gap: '64px', alignItems: 'start', flexWrap: 'wrap' }}>
                                        ...
                                        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '32px' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('business_name')}</label>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('corporate_email')}</label>
                                                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }} className="group">
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('phone_contact')}</label>
                                                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }} className="group">
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('physical_address')}</label>
                                                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }} className="group">
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '32px', paddingTop: '48px', borderTop: '1px solid #f1f5f9' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                            <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <Globe size={14} /> {t('operational_language')}
                                            </label>
                                            <select
                                                style={{
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                            <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <Timer size={14} /> {t('kitchen_status')}
                                            </label>
                                            <div
                                                onClick={() => setFormData({ ...formData, general: { ...formData.general, isKitchenOpen: !formData.general.isKitchenOpen } })}
                                                style={{
                                                    width: '100%', padding: '20px 32px', background: '#f8fafc', border: '2px solid transparent',
                                                    borderRadius: '24px', fontWeight: '700', color: '#1e293b', appearance: 'none',
                                                    transition: 'all 0.3s', outline: 'none', cursor: 'pointer',
                                            ...
                                            >
                                                <span>{formData.general.isKitchenOpen ? t('open') : t('closed')}</span>
                                                <div style={{
                                                    width: '48px', height: '24px', borderRadius: '12px',
                                                    background: formData.general.isKitchenOpen ? '#10b981' : '#ef4444',
                                                    position: 'relative', transition: 'background-color 0.3s'
                                                }}>
                                                    <div style={{
                                                        width: '20px', height: '20px', borderRadius: '50%', background: 'white',
                                                        position: 'absolute', top: '2px',
                                                        left: formData.general.isKitchenOpen ? 'calc(100% - 22px)' : '2px',
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }} className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                                    <div style={{ display: 'flex', gap: '48px', alignItems: 'start', flexWrap: 'wrap' }}>
                                                    ...
                                                    .map(curr => (
                                                        <button
                                                            ...
                                                            style={{
                                                                ...
                                                                padding: '20px',
                                                                borderRadius: '24px',
                                                                background: formData.general.currency === curr.code ? 'white' : 'transparent',
                                                                border: formData.general.currency === curr.code ? '2px solid #6366f1' : '2px solid transparent',
                                                                boxShadow: formData.general.currency === curr.code ? '0 10px 20px rgba(99, 102, 241, 0.1)' : 'none',
                                                                transition: 'all 0.3s ease',
                                                                cursor: 'pointer'
                                                            }}
                                                        >
                                                            <span style={{ fontSize: '24px', fontWeight: '900', color: formData.general.currency === curr.code ? '#4f46e5' : '#94a3b8' }}>
                                                                {curr.symbol}
                                                            </span>
                                                            <span style={{ fontSize: '14px', fontWeight: '800', color: formData.general.currency === curr.code ? '#1e293b' : '#64748b', marginTop: '4px' }}>
                                                                {curr.code}
                                                            </span>
                                        ...
                                        <div style={{ 
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                                <div style={{ padding: '12px', background: '#f5f3ff', color: '#7c3aed', borderRadius: '16px' }}>
                                                    <Coins size={24} />
                                                </div>
                                                <div>
                                                    <h4 style={{ fontSize: '16px', fontWeight: '900', color: '#1e293b', margin: 0 }}>{t('global_currency_selector')}</h4>
                                                    <p style={{ fontSize: '11px', color: '#94a3b8', margin: 0, fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Configuração Ativa</p>
                                                </div>
                                            </div>

                                            <div style={{ background: '#f8fafc', borderRadius: '32px', padding: '48px 24px', textAlign: 'center', border: '1px solid #f1f5f9' }}>
                                                <span style={{ fontSize: '64px', fontWeight: '900', color: '#6366f1', lineHeight: 1 }}>
                                                    {supportedCurrencies.find(c => c.code === formData.general.currency)?.symbol || '$'}
                                                </span>
                                                <h3 style={{ fontSize: '24px', fontWeight: '900', color: '#0f172a', marginTop: '16px', marginBottom: '4px', letterSpacing: '-0.02em' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }} className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '24px', flexWrap: 'wrap' }}>
                                        <div style={{ flex: 1, minWidth: '300px', maxWidth: '600px', position: 'relative' }} className="group">
                                            ...
                                            <input
                                                type="text"
                                                placeholder={t('filter_customers_placeholder')}
                                        ...
                                        <div style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em' }}>
                                            <span>{t('total_customers_db', { count: customers.filter(c =>
                                    ...
                                    {customerLoading ? (
                                        <div style={{ padding: '96px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                                            <LoadingSpinner />
                                            <p style={{ color: '#94a3b8', fontWeight: '700' }} className="animate-pulse">{t('crossing_sales_data')}</p>
                                        </div>
                                    ) : (
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '48px' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginLeft: '8px' }}>
                                                ...
                                                <h3 style={{ fontSize: '18px', fontWeight: '900', color: '#1e293b', letterSpacing: '-0.02em', margin: 0 }}>{t('currency_settings_title')}</h3>
                                            </div>
                                            ...
                                            <div style={{ padding: '40px', background: 'rgba(248, 250, 252, 0.5)', borderRadius: '48px', border: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                                    <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('base_currency')}</label>
                                                    <div style={{ position: 'relative' }}>
                                                        <input
                                                            type="text"
                                                            value={formData.general.currency}
                                                            onChange={e => setFormData({ ...formData, general: { ...formData.general, currency: e.target.value.toUpperCase() } })}
                                                            style={{
                                                                width: '100%', padding: '24px 32px', background: 'white', border: '1px solid #f1f5f9',
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginLeft: '8px' }}>
                                                ...
                                                <h3 style={{ fontSize: '18px', fontWeight: '900', color: '#1e293b', letterSpacing: '-0.02em', margin: 0 }}>{t('supported_currencies')}</h3>
                                            </div>

                                            <div style={{ padding: '40px', background: 'rgba(248, 250, 252, 0.5)', borderRadius: '48px', border: '1px solid #f1f5f9' }}>
                                                <div style={{ background: 'white', borderRadius: '32px', border: '1px solid #f1f5f9', overflow: 'hidden' }}>
                                                    {['MZN', 'USD', 'EUR', 'ZAR'].map((curr, idx) => (
                                                        <div key={curr} style={{ 
                                                            padding: '24px 32px', 
                                                            display: 'flex', 
                                                            alignItems: 'center', 
                                                            justifyContent: 'space-between',
                                                            borderBottom: idx === 3 ? 'none' : '1px solid #f8fafc',
                                                            background: formData.general.currency === curr ? 'rgba(var(--primary-rgb), 0.02)' : 'transparent'
                                                        }}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '48px' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                            ...
                                            <div style={{ padding: '40px', background: 'rgba(248, 250, 252, 0.5)', borderRadius: '48px', border: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '32px', background: 'white', borderRadius: '32px', border: '1px solid #f8fafc', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}>
                                                    <div>
                                                        <p style={{ fontWeight: '900', color: '#1e293b', margin: 0 }}>{t('auto_reset_table')}</p>
                                                        <p style={{ fontSize: '11px', fontWeight: '700', color: '#94a3b8', marginTop: '4px', margin: 0 }}>{t('auto_reset_table_desc')}</p>
                                                    </div>
                                                    <button
                                                        onClick={() => setFormData({ ...formData, flows: { ...formData.flows, autoResetTableOnPayment: !formData.flows.autoResetTableOnPayment } })}
                                                        style={{
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '32px', background: 'white', borderRadius: '32px', border: '1px solid #f8fafc', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)', opacity: 0.5, cursor: 'not-allowed' }}>
                                                    <div>
                                                        <p style={{ fontWeight: '900', color: '#1e293b', margin: 0 }}>{t('fee_calculation')}</p>
                                                        <p style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginTop: '4px', margin: 0 }}>{t('suggest_tip')}</p>
                                                    </div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginLeft: '8px' }}>
                                                ...
                                                <h3 style={{ fontSize: '18px', fontWeight: '900', color: '#1e293b', letterSpacing: '-0.02em', margin: 0 }}>{t('sla_patterns')}</h3>
                                            </div>

                                            <div style={{ padding: '40px', background: 'rgba(248, 250, 252, 0.5)', borderRadius: '48px', border: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                                    <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('idle_monitor')}</label>
                                                    <div style={{ position: 'relative' }} className="group">
                                                        ...
                                                        <input
                                                            ...
                                                            className="focus:ring-4 focus:ring-primary/5 shadow-sm"
                                                        />
                                                        <span style={{ position: 'absolute', right: '32px', top: '50%', transform: 'translateY(-50%)', fontWeight: '900', color: '#cbd5e1', fontSize: '10px', letterSpacing: '0.2em' }}>{t('minutes')}</span>
                                                    </div>
                                                    <p style={{ fontSize: '11px', fontWeight: '700', color: '#94a3b8', marginLeft: '16px', maxWidth: '320px', margin: 0 }}>{t('sla_desc')}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {(activeTab === 'security' || activeTab === 'integrations') && (
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '96px 0' }} className="animate-in zoom-in duration-700">
                                    ...
                                    <h3 style={{ color: '#1e293b', fontWeight: '900', fontSize: '30px', letterSpacing: '-0.02em', margin: 0 }}>{t('premium_feature')}</h3>
                                    <p style={{ color: '#94a3b8', font: 'bold', maxWidth: '448px', textAlign: 'center', marginTop: '16px', lineHeight: '1.6', fontSize: '18px', margin: 0 }}>
                                        {t('premium_desc', { tab: activeTab === 'security' ? t('tab_security') : t('tab_integrations') })}
                                    </p>
                        ...
                        <div style={{ padding: '32px 48px', background: '#0f172a', borderTop: '1px solid #1e293b', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '24px', position: 'relative', zIndex: 10 }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    <p style={{ fontSize: '13px', color: 'white', fontWeight: '900', letterSpacing: '-0.01em', margin: 0 }}>{t('secure_operation')}</p>
                                    <p style={{ fontSize: '10px', color: '#94a3b8', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.1em', margin: 0 }}>{t('admin_only_notice')}</p>
                                </div>
                            </div>
                            <div style={{ position: 'relative', zIndex: 10, display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <div style={{ width: '8px', height: '8px', background: '#34d399', borderRadius: '50%' }} className="animate-pulse" />
                                <span style={{ fontSize: '10px', fontWeight: '900', color: 'white', textTransform: 'uppercase', letterSpacing: '0.2em' }}>{t('secure_environment')}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', background: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(16px)' }} className="animate-in fade-in duration-500">
                    <div style={{ background: 'white', borderRadius: '64px', boxShadow: '0 64px 128px -32px rgba(0, 0, 0, 0.5)', width: '100%', maxWidth: '672px', overflow: 'hidden', border: '1px solid white' }} className="animate-in zoom-in duration-500">
                         <div style={{ padding: '48px 64px', borderBottom: '1px solid #f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(248, 250, 252, 0.3)' }}>
                             <div>
                                 <h2 style={{ fontSize: '36px', fontWeight: '900', color: '#1e293b', letterSpacing: '-0.05em', margin: 0 }}>{t('new_specialist')}</h2>
                                 <p style={{ color: '#94a3b8', fontSize: '12px', fontWeight: '700', marginTop: '8px', textTransform: 'uppercase', letterSpacing: '0.2em', margin: 0 }}>{t('new_specialist_desc')}</p>
                             </div>
                         ...
                         <form onSubmit={handleCreateUser} style={{ padding: '64px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                             <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                 <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('full_name')}</label>
                                 <input
                                     required
                                     style={{
                             ...
                             <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '32px' }}>
                                 <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                     <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('access_email')}</label>
                                     <input
                                         type="email" required
                                         style={{
                                             width: '100%', padding: '24px 32px', background: '#f8fafc', border: '2px solid transparent',
                                         ...
                                         className="focus:bg-white focus:border-primary/20 shadow-sm"
                                         placeholder={t('example_email_placeholder')}
                                         value={userFormData.email}
                                         onChange={e => setUserFormData({ ...userFormData, email: e.target.value })}
                                     />
                                 </div>
                                 <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                     <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('access_profile')}</label>
                                     <div style={{ position: 'relative' }}>
                                         <select
                                             required
                                             style={{
                             ...
                             <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                 <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '16px' }}>{t('temporary_credential')}</label>
                                 <div style={{ position: 'relative' }}>
                                 ...
                                 <p style={{ fontSize: '10px', font: 'bold', color: '#94a3b8', textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.2em', marginTop: '16px', margin: 0 }}>{t('password_change_notice')}</p>
                             </div>
